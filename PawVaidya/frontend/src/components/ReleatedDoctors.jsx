          ...
          {relDoc.slice(0 , 5).map((item , index)=>( 
            <div onClick={()=>{navigate(`/appointment/${item._id}`); scrollTo(0 , 0)}} className='border border-[#5A4035] rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500 ' key={index}>
              ...
              <div className='p-4 '>
                ...
                <p className='text-gray-600 text-sm '>{translateSpeciality(item.speciality, t)}</p>
              </div>
            </div>
          ))}
        </div>
      ) : (
        <p className='text-center font-extrabold text-4xl sm:text-5xl md:text-6xl lg:text-8xl text-green-400 px-4 sm:px-10'>🦥 No Other Doctor🦦 <br /> Available from  "{State}" <br /> for <br /> "{translateSpeciality(speciality, t)}" <br /></p>
      )}
    </div>
  )
}
